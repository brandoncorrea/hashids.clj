...
(defn consistent-shuffle
  ...
  (if (empty? salt)
    alphabet
    (apply str (reduce (fn [[alph p] [idx _a]]
                         (let [i (- (count alph) idx 1)
                               v (mod idx (count salt))
                               n (util/char-code (nth salt v))
                               p (+ p n)]
                           (if (zero? i)
                             alph
                             [(util/swap alph i (mod (+ n v p) i)) p])))
                       [(vec alphabet) 0]
                       (map-indexed (fn [idx a] [idx a]) alphabet)))))

(defn enhash
  ;; Known as 'hash' in other implementations
  ([input alphabet] (enhash input alphabet (long input) ""))
...
(defn dehash
  ...
  (reduce +
          (keep-indexed
            (fn [idx c]
              (when-let [pos (first (util/positions #{c} alphabet))]
                (* pos (long (math/pow (count alphabet)
                                       (- (count input) idx 1))))))
            (vec input))))
  ...
  (let [lottery    (str (nth alphabet (mod hash-int (count alphabet))))
        ...
        enc-result (reduce (fn [[alph ret] [idx n]]
                             (let [buf     (concat lottery salt alph)
                                   ...
                                   addsep  (nth sepsc (mod n (+ (util/char-code (first encchar)) idx)))]
...
(defn add-guards
  ...
  (let [prepend-guard #(if (< (count %) min-length)
                         (str (nth (cycle guards)
                                   (+ hash-int (util/char-code (nth % 0))))
                              %)
                         %)
        append-guard  #(if (< (count %) min-length)
                         (str %
                              (nth (cycle guards)
                                   (+ hash-int (util/char-code (nth % 2)))))
                         %)]
...
(defn balance-seps
  ...
  (let [seps-length-ceil (int (math/ceil (/ (count alph) SEP_DIV)))
        ...
        split-alph       (map str/join (split-at seps-diff alph))]
...
(defn decode
  ...
  (let [{:keys [seps alphabet salt guards]} (setup opts)
        breakdown      (util/split-on-chars encstr guards)
        breakdown-idx  (if (some #{(count breakdown)} '(2 3)) 1 0)
        bdn            (nth breakdown breakdown-idx)
        lottery        (first bdn)
        arr            (util/split-on-chars (drop 1 bdn) seps)
